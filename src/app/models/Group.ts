...
export default class Group {
  ...
  save = async () => {
    ...
    if (groupCollection === undefined) {
      throw new Error(
        'Attempted to save group but groupCollection is undefined'
      );
    }
    const find = await groupCollection.getById(this.contactId.user);
    if (find != null) {
      return await groupCollection.update(this.contactId.user, this);
    }
