...
export default class ClientHelpers {
  ...
  cleanupGroup = async (message: Message) => {
    ...
    if (groupCollection === undefined) {
      logger.warn(
        'Attempted to cleanup group but groupCollection is undefined'
      );
      return;
    }
    ...
    const group = await groupCollection.getById(chat.id.user);
  ...
  handleSignups = async (message: Message) => {
    ...
    if (userCollection === undefined) {
      logger.warn(
        'Attempted to handle signups but userCollection is undefined'
      );
      return;
    }
    if (groupCollection === undefined) {
      logger.warn(
        'Attempted to handle signups but groupCollection is undefined'
      );
      return;
    }
    await Promise.all([addUser(message), addGroup(message, this.client)]);
  };
  ...
  static getUserFromMessage = async (message: Message) => {
    ...
    if (userCollection === undefined) {
      logger.warn(
        'Attempted to get user from message but userCollection is undefined'
      );
      return null;
    }
    const contact = await message.getContact();
    return await userCollection.getById(contact.id.user);
  };
  ...
  static hasUserPermission = async (message: Message, restricted: boolean) => {
    ...
    if (user === null) {
      message.reply(
        '🐛 I could not find info about you and that is why I can not run this command right now.'
      );
      logger.warn(
        "Attempted to get user permission but user is null. Command WON'T be executed."
      );
      return false;
    }
    ...
    if (!fromAdmin) {
      logger.warn(
        "A non-admin user tried to execute a restricted command. Command WON'T be executed."
      );
      return false;
    }
    return true;
  ...
  static checkPermissions = async ({
    ...
    const userHasPermission = await ClientHelpers.hasUserPermission(
      message,
      restricted
    );
  ...
  didMessageMentionMe = async (message: Message) => {
    ...
    return mentions.some((mention) => mention.id.user === me.user);
  };
