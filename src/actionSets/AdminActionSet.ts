...
export default class AdminActionSet {
  ...
  addAdmin = async (message: Message, client: YagamiClient) => {
    try {
      ...
      const admin = await userCollection.getById(contact.id.user);
      if (admin === null) {
      ...
      if (admin?.isAdmin) {
        return await message.reply('This contact is already an admin.');
      }
      await userCollection.addAdmin(contact.id.user);
  ...
  removeAdmin = async (message: Message, client: YagamiClient) => {
    ...
    try {
      const admins: User[] = await userCollection.getAdmins();
      if (admins.length === 0) {
        return await message.reply('There are no admins registered yet.');
      }
      const { contact } = await chooseContact(client, message, {
        fromAnyUserInChat: true
      });
      const admin = admins.find(
        (admin) => admin.contactId.user === contact.id.user
      );
      if (admin == null || !admin.isAdmin) {
        return await message.reply('This contact is not an admin.');
      }
      await userCollection.removeAdmin(contact.id.user);
