...
export default class Yagami extends Client {
        ...
        this.on(
            ...
            async (commandExecuted: CommandExecuted, message: Message) => {
                ...
                if (commandExecuted.command.attributes.countAsCommandExecuted) {
                    UserCollection.increaseTotalCommandsCalled(
                        user.id._serialized
                    );
                    if (group.isGroup) {
                        GroupCollection.increaseTotalCommandsCalled(
    ...
    private logCommandExecuted(commandExecuted: CommandExecuted) {
        const commandTrigger =
            commandExecuted.command.attributes.trigger.mainText;
        ...
        remove(path.join("./", `.wwebjs_auth_${this.clientId}`), async () => {
            try {
                await copyDir(
                    path.join(
                        "./",
                        `.wwebjs_auth_${this.clientId}` + "_BACKUP"
                    ),
                ...
                this.initialize();
                logger.info(
                    `${this.clientId} is trying to initializing again...`
                );
            } catch (error) {
                logger.error(
                    `${this.clientId} ERROR RESTORING SESSION BACKUP`,
                    error
                );
            }
        });
